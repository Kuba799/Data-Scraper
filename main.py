...
def yield_best_odd(dictionary, number=3):
    top_list = []
    for i in range(number):
        top_list.append(next(next(yield_by_odds(dictionary))))
    return top_list
...
while True:
    ...
    for link in link_scraper.links:
        ...
        """
        Match = BestBet(get_match(driver), get_date(driver), yield_by_odds(dictionary_home)[0],
                        yield_by_odds(dictionary_home)[1], yield_by_odds(dictionary_draw)[0],
                        yield_by_odds(dictionary_draw)[1], yield_by_odds(dictionary_away)[0],
                        yield_by_odds(dictionary_away)[1])

        highest_margin = 1.022
        if Match.margin < highest_margin:
            print(Match.get_best_bet())
            if "Unibet" in {Match.book_home, Match.book_draw, Match.book_away}:
                print("***************************************!!!!!!!!!!!!!!!!!!!!!BET"
                      "!!!!!!!!!!!!!!!!!!!!!***************************************")
